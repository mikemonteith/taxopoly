  ...
  return (
    <div className="mx-auto flex max-w-6xl gap-10 px-4 py-10 sm:px-6 lg:px-8">
      ...
      <main className="min-w-0 flex-1">
        <header className="mb-12 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1.5 max-w-prose text-sm text-muted-foreground">
              Every UI primitive the simulator is built from, in one place. Built on
              Radix UI primitives for accessibility (keyboard nav, focus management,
              ARIA) and Tailwind CSS for styling — see{" "}
        ...
        <Section id="colors" title="Colors" description="Semantic tokens — swap the values in app.css and every component below updates.">
        ...
        <Section id="typography" title="Typography" description="One scale, used consistently — headings stay in the body sans, numerals stay tabular where they line up in columns.">
          <div className="flex flex-col gap-3">
            <h1 className="text-3xl font-semibold tracking-tight">Heading 1 — Monopoly Simulator</h1>
            <h2 className="text-2xl font-semibold tracking-tight">Heading 2 — Turn 42</h2>
            <h3 className="text-xl font-semibold tracking-tight">Heading 3 — Player standings</h3>
            <h4 className="text-lg font-semibold">Heading 4 — Recent activity</h4>
            ...
            <p className="text-sm text-muted-foreground">Muted text, for secondary detail and captions.</p>
            <p className="font-mono text-sm tabular-nums">$1,920.00 — tabular figures for columns of numbers.</p>
          </div>
        </Section>

        <Section id="buttons" title="Buttons & toggles" description="Six intents, four sizes, plus the icon-toggle pattern used for play/pause and speed controls.">
            ...
            <div className="flex flex-wrap items-center gap-6">
              ...
              <Field label="Players">
                <div className="flex items-center gap-3 rounded-lg border border-border px-3 py-1.5">
                  <Button size="icon" variant="ghost" className="size-6" aria-label="Remove player">
                    <Minus className="size-3.5" />
                  </Button>
                  <span className="w-4 text-center text-sm font-medium tabular-nums">4</span>
                  <Button size="icon" variant="ghost" className="size-6" aria-label="Add player">
        ...
        <Section id="badges" title="Badges" description="Status and identity chips. Semantic colors (success/warning/destructive) are reserved — never reused for arbitrary categories.">
        ...
        <Section id="forms" title="Form controls" description="Every field pairs with a <Label>, carries a visible focus state, and supports disabled / invalid states.">
          <div className="grid gap-8 sm:grid-cols-2">
            ...
            <Field label="Notes" htmlFor="notes">
              <Textarea id="notes" placeholder="Anything to remember about this run…" />
            </Field>

            <div className="flex flex-col gap-4 sm:col-span-2">
              <div className="flex items-center gap-2">
                <Checkbox id="auto-mortgage" defaultChecked />
                <Label htmlFor="auto-mortgage">Auto-mortgage when cash runs out</Label>
              </div>
              <div className="flex items-center gap-2">
                <Switch id="unlimited-houses" />
                <Label htmlFor="unlimited-houses">Unlimited houses (ignore bank supply)</Label>
              </div>
            </div>

            <fieldset className="flex flex-col gap-3 sm:col-span-2">
              <legend className="text-sm font-medium">Trading between players</legend>
              <RadioGroup defaultValue="off" className="flex flex-col gap-2">
                <div className="flex items-center gap-2">
                  <RadioGroupItem value="off" id="trade-off" />
                  <Label htmlFor="trade-off">Off — AI players never trade (default for now)</Label>
                </div>
                <div className="flex items-center gap-2">
                  <RadioGroupItem value="fair" id="trade-fair" disabled />
                  <Label htmlFor="trade-fair">Fair-value only (coming later)</Label>
                </div>
                <div className="flex items-center gap-2">
                  <RadioGroupItem value="free" id="trade-free" disabled />
                  <Label htmlFor="trade-free">Unrestricted (coming later)</Label>
                </div>
              </RadioGroup>
            </fieldset>

            <Field label={`Tax rate — 12%`} htmlFor="tax-rate" className="sm:col-span-2">
              <Slider id="tax-rate" defaultValue={[12]} max={50} step={1} />
            </Field>
          </div>
        </Section>

        <Section id="tabs" title="Tabs" description="Used for the Board / Players / Stats screens on mobile.">
          <Tabs defaultValue="board" className="max-w-md">
            ...
            <TabsContent value="board" className="rounded-lg border border-border p-4 text-sm text-muted-foreground">
              Board view — 40 tiles, live token positions.
            </TabsContent>
            <TabsContent value="players" className="rounded-lg border border-border p-4 text-sm text-muted-foreground">
              Player list — cash, net worth, strategy.
            </TabsContent>
            <TabsContent value="stats" className="rounded-lg border border-border p-4 text-sm text-muted-foreground">
            ...
            <div>
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button variant="outline" size="icon" aria-label="What is net worth?">
                    <Info />
                  </Button>
                </TooltipTrigger>
                <TooltipContent>Net worth = cash on hand + property value.</TooltipContent>
              </Tooltip>
            </div>
          </div>
        </Section>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  This clears every player's balance and returns all tokens to GO. This
                  can&apos;t be undone.
                </DialogDescription>
              </DialogHeader>
              <DialogFooter>
        ...
        <Section id="composed" title="Composed example" description="The primitives above, combined into a player card — the kind of thing the board screen is built from.">
          <Card className="max-w-sm">
            ...
            <CardContent className="flex flex-col gap-3">
              <div className="flex items-baseline justify-between">
                <span className="text-2xl font-semibold tabular-nums">$180</span>
              ...
              <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                <Dices className="size-3.5" /> Rolled 6 + 4 · landed on St. James Place
              </div>
            </CardContent>
...
function Swatch({ name, className }: { name: string; className: string }) {
  return (
    <div className="flex flex-col gap-2">
      <div className={cn("flex h-16 items-end rounded-lg p-2 text-xs font-medium", className)}>{name}</div>
    </div>
  );
}
...
function ThemeToggle() {
  ...
  return (
    <Button variant="outline" size="icon" onClick={toggle} aria-label="Toggle color theme">
      {isDark ? <Sun /> : <Moon />}
    </Button>
  );
}
